...
def generate_dynamic_content(num_videos=5):
  print(
      f"🕊️ Meminta Groq Llama-3.3 meracik {num_videos} naskah ayat Alkitab &"
      " kueri Pexels..."
  )
  ...
  history_context = (
      "\n".join(used_verses[-25:])
      if used_verses
      else "(Belum ada riwayat, buat topik bebas)"
  )
          ...
          messages=[
              {
                  "role": "system",
                  "content": (
                      "Anda adalah asisten AI rohani yang patuh pada format"
                      " instruksi."
                  ),
              },
  ...
  for i, chunk in enumerate(raw_text.split("---")):
    ...
    renungan = (
        "Tuhan tidak pernah meninggalkanmu berjalan sendirian di tengah badai"
        " kehidupan."
    )
    ...
    for line in lines:
      if line.startswith("REF:"):
        ...
        ayat = line.replace("AYAT:", "").strip()
      elif line.startswith("RENUNGAN:"):
        renungan = line.replace("RENUNGAN:", "").strip()
      elif line.startswith("CTA:"):
        cta = line.replace("CTA:", "").strip()
      elif line.startswith("PEXELS_QUERY:"):
        pexels_query = line.replace("PEXELS_QUERY:", "").strip()
...
def download_pexels_video(query, output_filename):
  ...
  url = (
      ...
      f"{urllib.parse.quote(query)}&orientation=portrait&per_page=5"
  )
  ...
  try:
    ...
    if "videos" in response and len(response["videos"]) > 0:
      ...
      hd_file = next(
          (v for v in video_files if v["quality"] == "hd"), video_files[0]
      )
      ...
      if (
          ...
          and os.path.getsize(output_filename) > 50000
      ):
  ...
  fallback_url = (
      "https://api.pexels.com/videos/search?query=peaceful+nature+sunset+drone&orientation=portrait&per_page=1"
  )
...
def render_short_video(
    bg_video_path, audio_path, item, output_video, index, bg_music_path=None
):
  print(f"[{index}/5] 🎬 Merakit video latar Pexels, Teks & Musik...")

  if not os.path.exists(audio_path) or os.path.getsize(audio_path) < 1000:
    raise Exception(f"File audio {audio_path} tidak valid atau kosong!")
  if (
      not os.path.exists(bg_video_path)
      or os.path.getsize(bg_video_path) < 50000
  ):
  ...
  video_clip = video_clip.resized(height=1920).cropped(
  ...
  progress_bar = ColorClip(
      size=(1080, 15), color=(255, 215, 0)
  ).with_duration(video_duration)
  progress_bar = progress_bar.with_position(
      lambda t: (int(-1080 + (1080 * (t / video_duration))), "bottom")
  )
  ...
  if file_size < 50000:
    ...
    raise Exception(
        f"File {output_video} gagal dibuat atau ukurannya korup/0 byte!"
    )
...
if __name__ == "__main__":
  ...
  if not os.path.exists(bg_music_file):
    print(
        "⚠️ Info: File 'bg_music.mp3' tidak ditemukan. Video akan berjalan"
        " tanpa musik latar."
    )
    bg_music_file = None
  ...
  for i, item in enumerate(generated_batch, 1):
    try:
      ...
      suara_naskah = (
          f"{clean_spoken_ref} \"{item['ayat']}\" {item['renungan']}"
          f" {item['cta']}"
      )
      ...
      caption = (
          f"📖 Renungan Harian Firman Tuhan: {item['ref']}\n\n\"{item['ayat']}\"\n"
          f"\n{item['renungan']}\n\n{item['cta']}\n\n#firmantuhan"
          " #renunganharian #ayatalkitab #rohanikristen #saatteduh #reels"
      )
      ...
      if os.path.exists(video_file) and os.path.getsize(video_file) > 50000:
        ...
        raise Exception(
            "File video hilang atau ukurannya 0 byte sebelum di-upload!"
        )
